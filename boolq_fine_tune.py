...
def _process_doc(doc,i):
    out_doc = {
        "question": doc["question"][i],
        "passage": doc["passage"][i],
        "answer": doc["answer"][i],
    }
    return out_doc
...
def formatting_prompts_func(examples):
    output_text = []
    for i, gold in enumerate(examples["question"]):
        doc = _process_doc(examples,i)
        response = doc["answer"]
        passage = doc["passage"]
        question = doc["question"]
        
        instruction_block = f"Passage: {passage}\n"
        instruction_block += f"Question: {question}\n"
        instruction_block += f"Answer: {response}"
...
OUTPUT_DIR = "tune_log/taylor/param_first/group_reduction/sum/0.5/boolq/train/boolq_B4_G16_R8_A16"
