...
def main(args):
    ...
    def _process_doc_hellaswag(doc):
        ctx = doc["ctx_a"] + " " + doc["ctx_b"].capitalize()
        out_doc = {
                "query": preprocess_hellaswag(doc["activity_label"] + ": " + ctx),
                "choices": [preprocess_hellaswag(ending).strip() for ending in doc["endings"]],
                "gold": doc["label"],
        }
        return out_doc
    ...
    def create_prompt_for_hellaswag(goal, choices, gold=None):
        if gold != None:
    ...
    def create_prompt_for_piqa(goal, choices, gold=None):
        if gold != None:
            prompt = f"Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.\n\n### Instruction:\n{goal}\n\n### Input:\n1. {choices[0]}\n2. {choices[1]}\n\n### Response:\n{gold}"
        else:
            prompt = f"Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.\n\n### Instruction:\n{goal}\n\n### Input:\n1. {choices[0]}\n2. {choices[1]}\n\n### Response:\n"
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        elif 'piqa' in args.data_path.lower():
            full_prompt = create_prompt_for_piqa(
                data_point["goal"],
                [data_point["sol1"], data_point["sol2"]],
                data_point["label"],
            )
        elif 'hellaswag' in args.data_path.lower():
            out_doc = _process_doc_hellaswag(data_point)
            full_prompt = create_prompt_for_hellaswag(
                out_doc["query"],
                out_doc["choices"],
                out_doc["gold"],
            )
        ...
        if not args.train_on_inputs:
            if 'piqa' in args.data_path.lower():
                user_prompt = create_prompt_for_piqa(
                data_point["goal"],[data_point["sol1"], data_point["sol2"]],None)
                
            elif 'hellaswag' in args.data_path.lower():
                out_doc = _process_doc_hellaswag(data_point)
                user_prompt = create_prompt_for_hellaswag(out_doc["query"],out_doc["choices"],None)
                # print(user_prompt)
            else:
    ...
    if 'hellaswag' in args.data_path.lower():
        new_train_dataset = data["train"]
        new_validation_dataset = data["validation"]

        new_train_dataset = new_train_dataset.select(range(39900))
        new_validation_dataset = new_validation_dataset.select(range(10040))
        
        data["train"] = new_train_dataset
        data["validation"] = new_validation_dataset
    ...
    if args.cache_dataset and os.path.exists('datasets/cache/{}.bin'.format(args.data_path)):
        ...
        val_data = {
            args.data_path: data["validation"].shuffle().map(generate_and_tokenize_prompt),
        }
    ...
    if 'piqa' in args.data_path.lower():
        columns_to_remove = ['goal', 'sol1', 'sol2', 'label']
        train_data = train_data.remove_columns(columns_to_remove)
        val_data[args.data_path] = val_data[args.data_path].remove_columns(columns_to_remove)
    elif 'hellaswag' in args.data_path.lower():
        columns_to_remove = ['activity_label', 'ctx', 'ctx_a', 'ctx_b','endings','ind','label','source_id','split','split_type']
        train_data = train_data.remove_columns(columns_to_remove)
        val_data[args.data_path] = val_data[args.data_path].remove_columns(columns_to_remove)
